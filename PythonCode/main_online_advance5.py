...
def online_interact(trainID, dictTrainStationsTheTrain, dictStationTrainDepartsTheTrain, sc, tr, yearForecast):
    ## given: trainID
    trainingDirectory = os.path.abspath(os.path.join(os.getcwd(), os.pardir))+'/onlineTraining/'
    trainingData = load(trainingDirectory+str(trainID)+'_'+str(trainID)+'.npy')
    ## intepolation
...
#        print stationExtractList
        ...
        if lengthList == 0:
            ...
            trainingDataRelated = load(trainingDirectory+str(trainCode)+'_'+str(trainID)+'.npy')
            ## intepolation
...
#        print 'trainIDSet final: ',trainIDSetFinal
    ...
    #            print 'STOP not enough training data'
                ...
                estInter, estSim, estTrue, MSEest, MSEestSim, estInterR, estSimR, estTrueR, MSEestR, MSEestSimR = online_interact(trainID, dictTrainStationsTheTrain, dictStationTrainDepartsTheTrain, sc, tr, yearForecast)

                if len(trainIDSetFinal) > 1 and (1-isnan(estInterR)) and (1-isnan(MSEestR)) and MSEestR<600 and MSEestSimR<600:
                    try:
                        listEst.append(estInter)
                        listEstSim.append(estSim)
                        listEstMSE.append(MSEest)
                        listEstSimMSE.append(MSEestSim)
                        listTrue.append(estTrue)
                            
                        listEstR.append(estInterR)
                        listEstSimR.append(estSimR)
                        listEstMSER.append(MSEestR)
                        listEstSimMSER.append(MSEestSimR)
                        listTrueR.append(estTrueR)
    ...
    plt.rc('xtick',labelsize=20)
    plt.rc('ytick',labelsize=20)
    plt.plot(listEstSimR,listTrueR,'o')
    plt.plot(range(61),range(61),'g')
    plt.xlabel('Estimated delay',fontsize=20)
    plt.ylabel('True delay',fontsize=20)  
    plt.xlim([0,60])
    plt.ylim([0,60])           
    plt.savefig(saveDirectoryFig+'fig_inter_online_sim.pdf',bbox_inches='tight')  
    plt.show()
    plt.clf()    
    ...
    plt.rc('xtick',labelsize=20)
    plt.rc('ytick',labelsize=20)
    plt.plot(range(len(listEstMSERRank)), listEstMSERRank, label = 'Online model (3)')                 
    plt.plot(range(len(listEstSimMSERRank)), listEstMSERRank, label = 'Online model (7)')                    
